...
file, and that marker used to be .dev-copy: the maintainer's own tooling notes,
which meant the shipped source (and the update window's banner) named a private
local file to every user.
...
def test_the_old_marker_no_longer_triggers_it(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(service, "APP_ROOT", tmp_path)
    (tmp_path / ".dev-copy").write_text("notes", encoding="utf-8")
    assert service.in_dev_environment() is False
...
def test_no_shipped_module_mentions_the_old_marker():
    ...
    for path in _sources():
        if ".dev-copy" in path.read_text(encoding="utf-8", errors="ignore"):
